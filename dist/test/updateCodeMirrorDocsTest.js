...
describe('updateCodeMirrorDocs', function () {
  ...
  it('adds new text', function () {
    ...
    updateCodeMirrorDocs_1.default(
      doc1,
      doc2,
      function () {
        return codeMirror
    ...
    assert_1.default.deepStrictEqual(codeMirror.getValue(), doc2.text.toString())
    updateCodeMirrorDocs_1.default(
      doc2,
      doc3,
      function () {
    ...
    assert_1.default.deepStrictEqual(codeMirror.getValue(), doc3.text.toString())
  })
  ...
  for (var n = 0; n < 10; n++) {
    it('works with random edits (fuzz test ' + n + ')', function () {
      ...
      assert_1.default.deepStrictEqual(doc.text.toString(), codeMirror.getValue())
    })
  }
})
function monkeyModify(doc) {
  ...
  if (Math.random() < 0.7) {
    ...
    doc = automerge_1.default.change(doc, function (editableDoc) {
      var _a
      ;(_a = editableDoc.text).insertAt.apply(_a, __spread([index], random_1.randomString(editLength).split('')))
    })
  } else {
    var endIndex_1 = Math.min(index + editLength, textLength - index)
    doc = automerge_1.default.change(doc, function (editableDoc) {
      editableDoc.text.deleteAt(index, endIndex_1)
    })
  }
  return doc
}
